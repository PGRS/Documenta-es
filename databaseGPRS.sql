...
-- Tabelas Inventario/Residuo q se concecta a tbl Gerador
...
acondicionamento varchar(200), -- Lista puxando informações
transporteExt varchar(200), -- descrição de como acontece o transporte
armazenamento varchar(200), -- descrição da armaz. do residuo
...
constraint pk_DetInv foreign key(idDetInv) references detInv(idDetInv));

create table estocTemp(
idPlanMov int,
idEstocTemp int auto_increment,
localEstoc varchar(100),
primary key(idEstocTemp),
constraint pk_PlanMov_Estoc foreign key(idDetInv) references detInv(idDetInv));
